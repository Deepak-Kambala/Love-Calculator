...
function exportHistoryToPDF() {
  ...
  if (h.length === 0) {
    alert('No history to export. Calculate some love matches first! ❤️');
    return;
  }
  ...
  try {
    ...
    doc.text('Love Alchemy - History Report', 105, 20, { align: 'center' });

    // Subtitle with emoji-like hearts
    doc.setFontSize(10);
    doc.setTextColor(100, 100, 100);
    doc.text('Your love compatibility calculations', 105, 28, { align: 'center' });

    // Horizontal line
    doc.setDrawColor(255, 122, 122);
    ...
    h.forEach((entry, index) => {
      ...
      doc.text(`${entry.name1} + ${entry.name2}`, 35, yPosition);

      // Percentage (right-aligned)
      doc.setFontSize(18);
      doc.setTextColor(255, 46, 99);
      doc.text(`${entry.percent}%`, 190, yPosition, { align: 'right' });
      ...
      doc.text(entry.msg, 35, yPosition);
      ...
      doc.setFontSize(9);
      doc.setTextColor(150, 150, 150);
      const dateStr = new Date(entry.t).toLocaleString();
      doc.text(dateStr, 35, yPosition);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setTextColor(150, 150, 150);
      doc.text(
        `Generated by Love Alchemy | Page ${i} of ${totalPages}`,
        105,
        pageHeight - 10,
        { align: 'center' }
      );
    ...
    const now = new Date();
    const filename = `Love-Alchemy-History-${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}.pdf`;

    // Save the PDF
    doc.save(filename);

    // Show success message
    alert('✨ History exported successfully to PDF!');
  } catch (error) {
...
exportHistoryPDF.addEventListener('click', () => {
  exportHistoryToPDF();
});
